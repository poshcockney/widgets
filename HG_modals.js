/* HG_modals.js - Injects Custom Modals & Logic for Humble Grape */
(function() {
    ...
    const css = `
      ...
      /* --- PDF & G-Drive & Matterport Specific Close Button (The Green Box) --- */
      .pdf-close-btn {
        border: none; 
        background: #0a463b; /* Match the green requested */
        cursor: pointer; 
        font-size: 30px; 
        line-height: 1; 
        color: #fff; 
      ...
      /* --- PDF Book Table Button (Solid Green) --- */
      .pdf-book-btn {
        position: absolute; 
        bottom: 20px; 
        left: 20px; 
      ...
      .pdf-book-btn:hover {
        background-color: #08362d;
        opacity: 1; /* Override default generic hover */
      }
      ...
      #klaviyo-box { max-width: 500px; background: #0a463b; }
       
      /* Mobile PDF Height Override */
      @media (max-width: 640px) {
        #gdrive-pdf-box { height: 80vh !important; }
        #matterport-modal-box { height: 85vh !important; } /* Taller for 3D tour on mobile */
      }
      ...
      <div class="custom-modal-container">
        <div id="gdrive-pdf-box" class="custom-modal-box">
          ...
          <a class="pdf-book-btn"
             href="https://www.sevenrooms.com/explore/humblegrape/reservations/create/search?venues=humblegrapebattersea%2Chumblegrapecanarywharf%2Chumblegrapecrouchend%2Chumblegrapefleetstreet%2Chumblegrapeislington%2Chumblegrapeliverpoolstreet%2Chumblegrapestmarylebow" 
             target="_blank" 
             rel="noopener">
             Book A Table
          </a>
    ...
    <div id="gform-modal" class="custom-modal" style="z-index: 999996;">
        <div class="custom-modal-backdrop close-trigger"></div>
        <div class="custom-modal-container">
            <div class="custom-modal-box" style="max-width: 700px; height: 85vh;">
                <div style="position: absolute; top: 12px; right: 22px; z-index: 10;">
                    <button class="custom-close-btn close-trigger" style="color: #555;">&times;</button>
                </div>
                <div style="flex-grow: 1; position: relative; background: #fff; overflow: auto; -webkit-overflow-scrolling: touch;">
                    <div id="gform-loader" class="custom-loader">Loading form...</div>
                    <iframe id="gform-frame" src="about:blank" style="width: 100%; height: 100%; border: none; position: relative; z-index: 1;"></iframe>
                </div>
            </div>
        </div>
    </div>
    ...
    container.id = 'hg-modals-root';
    container.innerHTML = html;
    document.body.appendChild(container);

    // --- 3. JS Logic Initialization ---
    function initModals() {
        /* === 1. CLOSE LOGIC === */
        function closeAllModals() {
            ...
            // Reset iframes
            ['pipedrive-frame', 'gdrive-pdf-frame', 'matterport-frame', 'gform-frame'].forEach(id => {
        ...
        /* === 6. GOOGLE FORM LOGIC === */
        const gFormLinks = document.querySelectorAll('a[href="#google-form-trigger"]');
        const gFormModal = document.getElementById('gform-modal');
        const gFormIframe = document.getElementById('gform-frame');
        const gFormLoader = document.getElementById('gform-loader');
        const gFormUrl = 'https://docs.google.com/forms/d/e/1FAIpQLSfvdyNNXPiUrLoocuX62mmKDOvOf1nPJhv6HIBUSfgdmkkWcw/viewform?embedded=true';

        function gFormIframeLoadHandler() {
            if (gFormLoader) gFormLoader.style.display = 'none';
            if (gFormIframe) gFormIframe.removeEventListener('load', gFormIframeLoadHandler);
        }

        gFormLinks.forEach(link => {
            link.addEventListener('click', function(e) {
                e.preventDefault();
                if (!gFormModal || !gFormIframe) return;
                  
                gFormLoader.style.display = 'block';
                gFormIframe.addEventListener('load', gFormIframeLoadHandler);
                gFormIframe.src = gFormUrl;
                  
                gFormModal.style.display = 'block';
                document.body.style.overflow = 'hidden';
            });
        });
    }
